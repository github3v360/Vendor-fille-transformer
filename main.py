...
def helloFirestore(event, context):
    ...
    print(metaData)
    print('========hello======')
    print(metaData['CREATEDAT'].strftime("%d/%m/%Y"))
    ...
    for everyobj in bucketPathArray:
        ...
        out_df = Extraction_of_entire_file.extract_entire_file(os.path.join(tempdir, currentFilePath.split('/')[-1]),False,logging,'temp.xlsx')
