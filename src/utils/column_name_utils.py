...
def get_standard_names(target_name,logger):
  ...
  if target_name == "clarity":
   return ["clarity","purity","Clar", "Clearity"]
    ...
    x =  ['REPORTNO','REPORT NO','REP NO','REPORT #','CERT#','CERTIFICATE','CERTIFICATE NO','CERTIFICATE #','CERTI NO.','REPORT','CERT #','CERT NO.','CERTNO','CERT. NO','GIA OR FM','REP_NO','CERT_NO','CERT.NO','VIEW CERTIFICATE','CERT NO','CERTINO']
    m = []
    for i in x:
        m.append(i + "_link")
    return m
