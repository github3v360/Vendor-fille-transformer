...
def csv_diff_of_each_cell(file1, file2):
    ...
    Returns:
    list: A list of tuples, where each tuple represents a cell that is different between the two files.
          The tuple contains the row index, column index, and the values from both files respectively.
    """
    ...
    for i in range(min(len(csv1), len(csv2))):
        for j in range(min(len(csv1[i]), len(csv2[i]))):
            if csv1[i][j] != csv2[i][j]:
                diff.append((i, j, csv1[i][j], csv2[i][j]))
...
def file_diff_of_each_line(file1, file2):
    ...
    Returns:
    list: A list of strings, where each string represents a line that is different between the two files.
    """
    ...
    while i < len(lines1) or i < len(lines2):
        if i >= len(lines1) or i >= len(lines2):
            diff.append(lines1[i] if i < len(lines1) else lines2[i])
        elif lines1[i] != lines2[i]:
            diff.append(f"< {lines1[i].strip()}\n> {lines2[i].strip()}\n")
        i += 1
